...
use tantivy::{
    collector::TopDocs, doc, query::QueryParser, schema::*, Document, Index, IndexReader,
    IndexWriter, ReloadPolicy, Searcher,
};
use uuid::Uuid;
...
pub fn init() -> tantivy::Result<TantivyIndex> {
    ...
    let index = Index::open_or_create(
        MmapDirectory::open(Path::new("search_index/")).unwrap(),
        schema.clone(),
    )?;
...
impl TantivyIndex {
    ...
    pub fn update_doc(&mut self, doc: Doc2Index) -> tantivy::Result<()> {
        ...
        let _n = self
            .writer
            .delete_term(Term::from_field_text(article_id, &doc.article_id));
    ...
    pub fn delete_doc(&mut self, doc_id: &str) -> tantivy::Result<()> {
        ...
        let _n = self
            .writer
            .delete_term(Term::from_field_text(article_id, doc_id));
        ...
        for (_, doc_address) in doc_addresses {
            ...
            let new_doc = DocFromIndexOuter {
                ...
                created_time: DateTime::<Utc>::from_utc(
                    NaiveDateTime::from_timestamp(created_timestamp, 0),
                    Utc,
                ),
...
pub fn run_tantivy(
    tan_index_rx: Receiver<(TanAction, String, Option<Doc2Index>)>,
    tan_query_tx: Sender<Vec<DocFromIndexOuter>>,
) {
    ...
    thread::spawn(move || loop {
        match tan_index_rx.recv() {
            Ok((action, head_data, doc)) => match action {
                TanAction::Add => {
                    if doc.is_some() {
