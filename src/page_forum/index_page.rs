use log::info;
use sapper::{
    status, Module as SapperModule, Request, Response, Result as SapperResult,
...
use crate::middleware::{check_cache_switch, permission_need_be_admin};
use crate::tantivy_index::{Doc2Index, DocFromIndexOuter, TanAction};
...
impl IndexPage {
    ...
    pub fn search_query_page(req: &mut Request) -> SapperResult<Response> {
        ...
        if q != "" {
            ...
            tan_index
                .send((TanAction::Query, q.to_string(), None))
                .unwrap();
...
impl SapperModule for IndexPage {
    ...
    fn router(&self, router: &mut SapperRouter) -> SapperResult<()> {
        ...
        router.get(
            ...
            Self::latest_reply_articles_paging,
        );
        router.get(
            "/latest_blog_articles_paging",
            Self::latest_blog_articles_paging,
        );
